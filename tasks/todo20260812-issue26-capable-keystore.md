...
| Finding | Verdict | Fix |
|---|---|---|
| R1 BBS self-certification | Valid. My earlier hardening fake lied only in `Sign` — exactly the check a provider lying in both halves defeats. | Advertise BBS only when the configured producer and independent internal `DefaultBbsCryptoProvider` verifier are both available; no same-provider fallback. Mirrors the ECDSA `VerificationProvider`. |
| R2 import backend-exception leak | Valid. Only `ArgumentException` was mapped inside the consume reader; later review showed blanket type filters still leaked generator-originated ODE/OCE/internal argument faults. | Normalize at the exact generator call site: only `ArgumentException("privateKey")` is caller-attributable; all other generator failures are `KeyStoreException(Unavailable)`. Material remains consumed because the store saw the secret. |
...
| R4 BBS count OOM | Valid. `new byte[count][]` allocated from an untrusted `Count`; `int.MaxValue` → `OutOfMemoryException`, and a byte bound cannot limit zero-byte messages. Follow-up found the byte total was also checked only after every message was copied. | `MaxBbsMessageCount = 4096` checked before allocation; header and cumulative message lengths are rejected before copying; inconsistent `Count`/indexer shape is `ArgumentException("request")`. PRD rule 9 extended. |
| R5 mutable BBS verification evidence | Valid. `IReadOnlyList<byte[]>` left each nested array mutable, so a hostile producer could rewrite a message, sign the rewrite, and make the independent verifier check the rewrite. | Preserve the validated deep snapshot for verification and give the producer a separate deep copy. |
| R6 inconsistent generator pair | Valid. A generator could return key A's public half with key B's same-type private half; generate/import committed it and failed only on first use. | Independently derive the returned private half with `DefaultKeyGenerator` and compare public bytes before commit; mismatch is `Unavailable`. |
